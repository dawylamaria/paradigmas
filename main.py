...
#
for linha in calibracao.strip().splitlines():
    # Encontra o primeiro e o último dígito na linha
    ...
    for char in linha:
        if char.isdigit():
            if primeiro_digito is None:
                primeiro_digito = char
            ultimo_digito = char


    if primeiro_digito is not None and ultimo_digito is not None:
        valor_calibracao = int(primeiro_digito + ultimo_digito)
        soma_calibracao += valor_calibracao

print(soma_calibracao)